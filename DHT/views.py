...
def sendtele(dht):
    token = '6912252558:AAFwkCBFZ3cGevE8IwG-ML1ETiMhM0BKw5U'
    rece_id = 5949567783
    bot = telepot.Bot(token)
    message = f' Ugent ! la température dépasse la normale \n'
    message += f'TEMP: {dht.temp} HUM: {dht.hum} Date: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'
    bot.sendMessage(rece_id, message)

def sendwhatsap():
    # Alertwhatsapp
    account_sid = 'ACf64d7421846731242b2bd8d94e660377'
    auth_token = '532f797a4ae5373bfa0ddc440dba8124'
    client = Client(account_sid, auth_token)
    message = client.messages.create(
        from_='whatsapp:+14155238886',
        body='La température nest pas dans la zone normale',
        to='whatsapp:+212620296894'
    )
